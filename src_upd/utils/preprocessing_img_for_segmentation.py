...
def process(data_path="", size=512):
    ...
    # Удаляю верхнюю и нижнюю часть изображения при большом размере входных данных
    if len(lst_files) > 2000:
        delta = (len(lst_files) - 2000) // 2
        lst_files = lst_files[delta + 150:-delta - 150]
        print('YA TYT', len(lst_files))
    if len(lst_files) > 1400:
        lst_files = lst_files[150:-150]
    elif len(lst_files) > 1000:
        lst_files = lst_files[100:-100]


    # По изображениям большого размера иду с большим шагом
    if len(lst_files) > 2000:
        step = len(lst_files) // 300
    elif len(lst_files) > 1500:
        step = 4
    elif len(lst_files) > 1000:
        step = 3
...
def create_3D_segmentations(filepath, downsample_rate=1):
    ...
    if img.shape[2] > 2000:
        step = int(img.shape[2] // 300)
    elif img.shape[2] > 1500:
        step = 4
    elif img.shape[2] > 1000:
        step = 3
    elif img.shape[2] > 600:
        step = 2
    else:
        step = 1
